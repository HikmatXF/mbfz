...
def loginCookies(PukimakBangetKamuAnjink):
	...
	try:
		requests.Session().headers.update({'content-type': 'application/x-www-form-urlencoded'})
		data = {'access_token': '1348564698517390|007c0a9101b9e1c8ffab727666805038','scope': ''}
		response = json.loads(requests.Session().post('https://graph.facebook.com/v2.6/device/login/', data = data).text)
		code, user_code = response['code'], response['user_code']
		verification_url, status_url = ('https://m.facebook.com/device?user_code={}'.format(user_code)), ('https://graph.facebook.com/v2.6/device/login_status?method=post&code={}&access_token=1348564698517390%7C007c0a9101b9e1c8ffab727666805038&callback=LeetsharesCallback'.format(code))
		requests.Session().headers.pop('content-type')
		requests.Session().headers.update({'sec-fetch-mode': 'navigate','user-agent': 'Mozilla/5.0 (Linux; Android 9; RMX1941 Build/PPR1.180610.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/107.0.5304.54 Mobile Safari/537.36','sec-fetch-site': 'cross-site','Host': 'm.facebook.com','accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','sec-fetch-dest': 'document',})
		response2 = requests.Session().get(verification_url, cookies = {'cookie': cookie_anda}).text
		if 'Bagaimana Anda ingin masuk ke Facebook?' in str(response2) or '/login/?next=' in str(response2):
			...
			requests.Session().headers.update({'origin': 'https://m.facebook.com','referer': 'https://m.facebook.com/device?user_code={}'.format(user_code),'content-type': 'application/x-www-form-urlencoded','sec-fetch-site': 'same-origin',})
			response3 = requests.Session().post('https://m.facebook.com{}'.format(action), data = data, cookies = {'cookie': cookie_anda})
			if 'https://m.facebook.com/dialog/oauth/?auth_type=rerequest&redirect_uri=' in str(response3.url):
				requests.Session().headers.pop('content-type');requests.Session().headers.pop('origin')
				response4 = requests.Session().post(response3.url, data = data, cookies = {'cookie': cookie_anda}).text
				action = re.search('action="(.*?)"', str(response4)).group(1).replace('amp;', '')
				requests.Session().headers.update({'origin': 'https://m.facebook.com','referer': response3.url,'content-type': 'application/x-www-form-urlencoded',})
				...
				response5 = requests.Session().post('https://m.facebook.com{}'.format(action), data = data, cookies = {'cookie': cookie_anda}).text
				windows_referer = re.search('window.location.href="(.*?)"', str(response5)).group(1).replace('\\', '')
				requests.Session().headers.pop('content-type');requests.Session().headers.pop('origin')
				requests.Session().headers.update({'referer': 'https://m.facebook.com/',})
				response6 = requests.Session().get(windows_referer, cookies = {'cookie': cookie_anda}).text
				if "Sukses!" in str(response6):
					requests.Session().headers.update({'sec-fetch-mode': 'no-cors','referer': 'https://graph.facebook.com/','Host': 'graph.facebook.com','accept': '*/*','sec-fetch-dest': 'script','sec-fetch-site': 'cross-site',})
					response7 = requests.Session().get(status_url, cookies = {'cookie': cookie_anda}).text
					access_token = re.search('"access_token": "(.*?)"', str(response7)).group(1)
					print(" [√] cookie anda valid ")
					turu(2)
					open("cookie.txt","w").write(cookie_anda);open("token.txt","w").write(access_token);utamaMenu("Masa Sih")
				else:
					print(" [!] kesalahan saat convert token ")
					exit()
